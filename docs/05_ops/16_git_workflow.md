...
- **開発中のコミットは雑でOK**（マージポイントで区切られ、`git log --graph`で追跡可能）
...
- デモ後の分岐
  - **OK（受け入れ）**: PRを Merge commit → `main` が「デモ済み最新版」になる
...
顧客に見せた状態を後から再現できるように、節目でタグを打つ（マージ後の `main` に打つのが簡単）。
...
Merge commitでマージポイントが記録されるため、**開発中は深く考えなくていい**。
`git log --graph` でブランチの分岐・合流が視覚的にわかる。
...
PRタイトルは `Merge pull request #XX` の横に表示され、履歴を追跡する際の目印になる。
...
rebaseでコンフリクトが起きても、最終的にMerge commitするので途中の履歴は気にしなくていい。
...
会議で決まった内容は「すぐに共有できる状態」にしておくことを優先する。
以下を満たす場合は `main` 直pushを許可する。

- 変更が `docs/00_overview/` または `docs/03_spec/` のみ
- Markdownのみ（コード・設定ファイルは含めない）
- 実装変更が必要な場合は**別PRで対応**する
- コミットメッセージは `docs:` で開始し、会議日を含める（例: `docs: MTG 2025-12-19 仕様反映`）

### PR運用（ソロでも必須）

確認者が自分だけでも、PRを残すと「差分の説明」「作業単位の境界」「後からの振り返り」ができる。

#### マージ方式: Merge commit（固定）

```
開発中（PR内）              マージ後（main）
--------------------------  →  -----------------------------------------
wip                              *   Merge pull request #12 (feat: ダッシュボード実装)
動いた                           |\
やっぱ変更                       | * wip
バグ直した                       | * 動いた
インデント                       | * やっぱ変更
...
- ブランチ履歴がそのまま残る
- `git log --graph` でマージポイントと分岐が視覚的にわかる
- **だから開発中のコミットは雑でいい**（後から追跡可能）
