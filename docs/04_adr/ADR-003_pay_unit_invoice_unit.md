...
### 運用制約: pay_unit = invoice_unit の強制同期（2025年1月追加）

**重要**: 実運用において `pay_unit` は常に `invoice_unit` と同じ値を強制設定する。

- **背景**: 請求区分と給与区分を異なる値にすると、顧客への請求とスタッフへの支払いに差額が生じ、不正な中抜きの温床となる可能性がある（搾取防止）
- **実装**: `AssignmentService.saveAssignments` で `pay_unit = invoice_unit` を強制代入
- **単価オーバーライド無効化**: `wage_rate` / `invoice_rate` は配置保存時に常に `null` を設定し、マスタ参照を強制
- **調整が必要な場合**: 支払画面/請求画面の調整額フィールドを使用し、明示的な監査記録を残す

> カラムとしては分離して保持するが、値は常に同期される。将来的に分離が必要になった場合に備えてスキーマは維持する。
