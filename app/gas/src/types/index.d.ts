...
declare global {
  ...
  // === 未移行サービスのambient宣言 ===
  const StatsService: {
    updateMonthlyStats(year: number, month: number): { success: boolean; error?: string; stats?: Record<string, unknown>; created?: boolean };
    _aggregateByCustomerYearly(years: number): unknown;
  };
